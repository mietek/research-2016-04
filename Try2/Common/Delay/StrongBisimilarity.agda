module Try2.Common.Delay.StrongBisimilarity where
...
mutual
  bind-assoc : ∀ {i A B C} {f : A → Delay ∞ B} {g : B → Delay ∞ C}
      → (a? : Delay ∞ A)
      → ((a? >>= f) >>= g) ≈⟨ i ⟩≈ (a? >>= λ a → f a >>= g)
...
mutual
  bind-cong-l : ∀ {i A B} {a? a?′ : Delay ∞ A} {f : A → Delay ∞ B}
      → a? ≈⟨ i ⟩≈ a?′
      → (a? >>= f) ≈⟨ i ⟩≈ (a?′ >>= f)
...
mutual
  bind-cong-r : ∀ {i A B} {f g : A → Delay ∞ B}
      → (a? : Delay ∞ A)    → (∀ a → (f a) ≈⟨ i ⟩≈ (g a))
      → (a? >>= f) ≈⟨ i ⟩≈ (a? >>= g)
...
sym-bind-assoc : ∀ {i A B C} {f : A → Delay ∞ B} {g : B → Delay ∞ C}
    → (a? : Delay ∞ A)
    → (a? >>= λ a → f a >>= g) ≈⟨ i ⟩≈ ((a? >>= f) >>= g)
